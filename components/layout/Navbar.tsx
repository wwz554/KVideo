...
export function Navbar({ onReset, isPremiumMode = false }: NavbarProps) {
    ...
    useEffect(() => {
        ...
        return () => {
            window.removeEventListener('kvideo-session-changed', refreshSession);
                ...
                <div className="bg-[var(--glass-bg)] border border-[var(--glass-border)] shadow-[var(--shadow-sm)] px-3 sm:px-6 py-2 sm:py-4 rounded-[var(--radius-2xl)]">
                    <div className="flex items-center justify-between gap-2 sm:gap-4">
                        ...
                        <div className="flex items-center gap-2 sm:gap-3 flex-shrink-0">
                            ...
                            {session && (
                                <div className="flex items-center gap-1 sm:gap-2">
                                    <div className="hidden sm:flex items-center gap-1.5 px-2.5 py-1 bg-[var(--glass-bg)] border border-[var(--glass-border)] rounded-[var(--radius-full)] text-xs">
                                        <div className="w-5 h-5 rounded-[var(--radius-full)] bg-[var(--accent-color)]/10 flex items-center justify-center text-[var(--accent-color)] font-bold text-[10px] border border-[var(--glass-border)]">{session.name.charAt(0)}</div>
                                        <span className="text-[var(--text-color)] max-w-[60px] truncate">{session.name}</span>
                                        {(session.role === 'admin' || session.role === 'super_admin') && (
                                            <span className="px-1 py-0.5 bg-[var(--accent-color)]/10 text-[var(--accent-color)] rounded text-[10px] font-medium">{session.role === 'super_admin' ? '超管' : '管理'}</span>
                                        )}
                                    </div>
                                    <button onClick={handleLogout} className="w-8 h-8 flex items-center justify-center rounded-[var(--radius-full)] bg-[var(--glass-bg)] border border-[var(--glass-border)] text-[var(--text-color-secondary)]" aria-label="退出登录" title="退出登录">
                                        <LogOut size={14} />
                                    </button>
                                </div>
                            )}
                            ...
                            <Link href={favoritesHref} className="w-8 h-8 sm:w-10 sm:h-10 flex items-center justify-center rounded-[var(--radius-full)] bg-[var(--glass-bg)] border border-[var(--glass-border)] text-[var(--text-color)]" aria-label="我的收藏" data-focusable>
                                <Icons.Heart size={20} />
                            </Link>

                            {isAdmin && (
                                <Link href={settingsHref} className="w-8 h-8 sm:w-10 sm:h-10 flex items-center justify-center rounded-[var(--radius-full)] bg-[var(--glass-bg)] border border-[var(--glass-border)] text-[var(--text-color)]" aria-label="设置" title="设置" data-focusable>
